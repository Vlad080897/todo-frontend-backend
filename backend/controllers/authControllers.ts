...
export const handleError = (err: CredentialsErrorType) => {
  ...
  if (err.message.includes("User validation failed")) {
    Object.values(err.errors).forEach((prop) => {
      errors[(prop as unknown as PropertiesType).path] = (
        prop as unknown as PropertiesType
      ).message;
    });
  }
  return errors;
...
export const getUser = (req: Request, res: Response<UserType | null>) => {
  ...
  if (token) {
    jwt.verify(
      token,
      "asfasfasfsdnlsbkbqwi",
      async (err: Error, decodedToken: { id: string }) => {
        if (err) {
...
export const loginPost = async (
  ...
  res: Response<{ user: UserType; refreshToken: string } | { error: string }>
) => {
  ...
  try {
    ...
    res
      ...
      .json({
        error:
          "Invalid email or pass. If you don't have an account you can sign up",
      });
  }
};
...
export const getNewToken = (
  ...
  res: Response<{ id: string } | { user: null } | { error: string }>
) => {
  ...
  if (refreshToken) {
    jwt.verify(
      refreshToken,
      "1234test",
      (err: Error, decodedToken: { user: UserType }) => {
        if (err) {
